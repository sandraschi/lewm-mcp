...
const BACKEND_NAME: &str = "lewm-mcp-backend.exe";
const BACKEND_PORT: u16 = 10927;
...
fn free_port(port: u16) -> bool {
    let kill_cmd = format!(
        "Stop-Process -Name 'lewm-mcp-backend' -Force -ErrorAction SilentlyContinue; \
         Stop-Process -Name 'lewm-mcp-native' -Force -ErrorAction SilentlyContinue; \
         taskkill /F /IM lewm-mcp-backend.exe /T 2>$null; \
         taskkill /F /IM lewm-mcp-native.exe /T 2>$null"
    );
        ...
        if i == 15 {
            let elevated = format!(
                "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                 '-NoProfile -Command \"Stop-Process -Name lewm-mcp-backend -Force -ErrorAction SilentlyContinue; \
                 taskkill /F /IM lewm-mcp-backend.exe /T 2>$null; \
                 Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                 ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
            );
