...
git clone https://github.com/sandraschi/lewm-mcp
cd lewm-mcp
just
```
...
4. Start the server:
   ...
   uv run python -m lewm_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn lewm_mcp.server:app --port 10927
   ```
...
5. Open `http://localhost:10927` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/lewm-mcp/issues) |
